...
class Events(commands.Cog):
    ...
    async def on_ready(self):
        await services.logger("BOT_CLIENT", Bcolors.GREEN, f"Logged in as: {self.client.user.name}")
        await services.logger("BOT_CLIENT", Bcolors.GREEN, f"Bot ID: {self.client.user.id}")
        await services.logger("BOT_CLIENT", Bcolors.YELLOW, "------")
        for guild in self.client.guilds:
            await services.logger(guild, Bcolors.YELLOW, "Connected.")
    ...
    async def on_guild_remove(self, guild):
        await services.logger("BOT_CLIENT", Bcolors.YELLOW, "------")
        await services.logger(guild, Bcolors.YELLOW, "Disconnected.")
        await services.logger("BOT_CLIENT", Bcolors.YELLOW, "------")
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await services.logger(ctx.guild, Bcolors.RED, f"'{ctx.message.content}' resulted in;\n{error}")

    # Member interaction
    @commands.Cog.listener()
    async def on_member_join(self, member):
        await services.logger(member.guild, Bcolors.LIGHT_BLUE, f"{member} joined.")
        if not member.bot:
    ...
    async def on_member_remove(self, member):
        await services.logger(member.guild, Bcolors.LIGHT_BLUE, f"{member} left.")
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if reaction.message.author is not user and reaction.message.author.bot is False and user.bot is False:
            ...
            try:
                ...
                await services.logger(user.guild, Bcolors.RED, f"{e}\nIn events.py : on_reaction_add")
            ...
            if dbfunctions.check_reaction(str(emoji_id), guild_id):
                ...
                if dbfunctions.set_karma_event(reaction.message.channel, user, reaction.message.author, guild_id):
                    await services.logger(user.guild, Bcolors.YELLOW, f"{user} gave {reaction.message.author} karma.")
